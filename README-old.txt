...
使用前说明:
1. 待替换数量不能超过服务器bg数量, 否则无法成功替换!
2. 如果想要一劳永逸,那就把 OsuWRS.exe/OsuWRS.py 当作osu!的启动器,osuWRS会自动检测ppy的更新

*使用方法:
1.将待替换图片放入bg目录下(只接受jpg/png格式)
2.将osu的绝对路径放入config.ini(见获取方法)
3.确保osu设置成下面这样子
  (Seasonal backgrounds - Always)
4.运行OsuWRS.exe/OsuWRS.py
